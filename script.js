...
function openQrModal(productId) { if (!qrModalTitle || !qrCodeDisplay || !qrCodeModal) return; const product = allProducts.find(p => p.id === productId); if (!product) { alert("Error: Producto no encontrado para QR."); return; } qrModalTitle.textContent = `QR: ${product.nombre}`; while (qrCodeDisplay.firstChild) { qrCodeDisplay.removeChild(qrCodeDisplay.lastChild); } const priceText = formatPrice(product.precioVenta, false); const qrText = `Producto: ${product.nombre}\nPrecio: $${priceText}`; try { if (typeof QRCode === 'undefined') throw new Error("QRCode lib not found."); new QRCode(qrCodeDisplay, { text: qrText, width: 256, height: 256, colorDark: "#000000", colorLight: "#ffffff", correctLevel: QRCode.CorrectLevel.H }); console.log("QR generated:", product.nombre); qrCodeModal.style.display = 'block'; } catch (error) { console.error("QR error:", error); alert("Error al generar QR."); qrCodeDisplay.innerHTML = '<p style="color:red;">Error QR.</p>'; qrCodeModal.style.display = 'block'; } }
